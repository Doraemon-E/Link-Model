...
def run_system(
    ...
    loaded_runtime = TranslationRuntime.load(
        artifact_stage_directory(config.artifacts_root, "quantized", "m2m100-418m")
    )
    ...
    manifest = load_artifact_manifest(artifact_manifest_path(config.artifacts_root, "m2m100-418m"))
...
def _warmup(config: BenchmarkConfig, runtime: TranslationRuntime, route: RouteSpec, text: str) -> None:
